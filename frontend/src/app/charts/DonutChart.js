...
class DonutChart extends Component {
  ...
  createDonutChart() {
    ...
    var color = scaleOrdinal().range([this.props.color, '#70758c']);

    var opacity = scaleOrdinal().range(['1', '0.2']);
    ...
    select(node)
      ...
      .attr(
        'transform',
        `translate(${2 * (margin.left + margin.right)}, ${
          1.8 * (margin.top + margin.bottom)
        })`
      )
      .attr('d', arcGenerator)
    ...
    select(node)
      ...
      .textTween(function (d) {
        ...
        return function (t) {
          return formating((val = Math.ceil(i(t)))) + '%';
        };
      });
    ...
    if (this.props.name === undefined) {
      ...
      select(node)
        ...
        .attr(
          'transform',
          `translate(${2 * (margin.left + margin.right)}, ${
            height - 1.5 * (margin.bottom + margin.top)
          })`
        )
