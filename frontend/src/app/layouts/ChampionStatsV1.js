...
const ChampionStatsV1 = (props) => {
  ...
  useEffect(() => {
    ...
    if (championName) {
      axios
        .get(
          `${baseUrl}/lol/champs/summary?champion=${championName}&region=${
            state?.filters?.region || 1
          }&fgm=${state?.filters?.fgm || 1}&duration=${
            state?.filters?.duration || 1
          }&tier=${state?.filters?.tier || 1}&role=${state?.filters?.role || 1}`
        )
        .then((response) => {
        ...
        <div className='container' style={{ position: 'relative' }}>
          {!Loading && championObj?.champion ? (
                ...
                <div className='mobalysis_detials'>
                  <div className='mobalysis_container'>
                    ...
                    <div className='mobalysis_container-roles'>
                      <RolesPerformance
                        rolePerformance={championObj?.role_performance}
                      />
                    </div>
                  </div>
                  ...
                  <div className='mobalysis_container_section-two'>
                    ...
                    <div className='mobalysis_conatiner-champ'>
                      <BestChamp
                        summoners={championObj?.best_players}
                        champion={championObj?.champion}
                      />
                    </div>
                  </div>
                </div>
            ...
            <ReactBootstrap.Spinner
              animation='border'
              className='spinner'
              style={{ position: 'absolute', top: '1rem', left: '35rem' }}
            />
          )}
        </div>
      </div>
